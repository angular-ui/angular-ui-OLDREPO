describe('format', function() {
  ...
  it('Should return only object with id < 2', function(){
      expect(dataFilterFilter(sampleData, {id__lt:2})).toEqual(
          [sampleData[0]]
      )
  });
  // lte
  it('Should return only objects with id <= 2', function(){
      expect(dataFilterFilter(sampleData, {id__lte:2})).toEqual(
          [sampleData[0], sampleData[1]]
      )
  });
  // gt
  it('Should return only objects with id > 12', function(){
      expect(dataFilterFilter(sampleData, {id__gt:12})).toEqual(
          [sampleData[12]]
      );
  });
  // gte
  it('Should return only objects with id >= 12', function(){
      expect(dataFilterFilter(sampleData, {id__gte:12})).toEqual(
          [sampleData[11], sampleData[12]]
      );
  });
  // in
  it('Should return only objects with id in [7,10,12]', function() {
      expect(dataFilterFilter(sampleData, {id__in:[7,10,12]})).toEqual(
